type MonoCondition = proc(map: FingerMap, row0, col0: uint8): bool {.closure.}

proc processMono(map: FingerMap, stat: string, condition: MonoCondition) =
  var monoStat = MonoStat(ngrams: newSeq[uint8](), weight: -Inf)
...
proc initializeMonoStats(map: FingerMap) =
  const
    ...
    rowStatNames =
      {0: "Top Row Usage", 1: "Home Row Usage", 2: "Bottom Row Usage"}.toTable

    handStatNames = {Left: "Left Hand Usage", Right: "Right Hand Usage"}.toTable
